...
parser.add_argument("--data_dir", default="/allen/programs/braintv/workgroups/nc-ophys/max.weil/data/data/nets/", type=str, help="Directory to pull data from")
parser.add_argument("--save_dir", default="/allen/programs/braintv/workgroups/nc-ophys/max.weil/data/data/dataframes/", type=str, help="Directory to save dataframe")
...
def main(net_name, dataset, data_dir, save_dir, epoch_num):
    ...
    for roots, dirs, files in os.walk(root):
        for file in files:
            if end in file:
                name = file.removeprefix(start).split('_')
